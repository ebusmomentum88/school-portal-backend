...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error("Please set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY in Render env vars.");
  process.exit(1);
}
...
async function generateStudentUsername() {
  const { data } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (data && data.student_number) studentCounter = parseInt(data.student_number) + 1;
...
app.post("/api/create-student", async (req, res) => {
  try {
    ...
    const { data: student, error: studentErr } = await supabase
      .from("students")
      .insert({ first_name, surname, class_id, gender, student_number: username })
      .select().single();
    if (studentErr) return res.status(500).json({ error: studentErr.message });

    const { error: appUserErr } = await supabase
      .from("app_users")
      .insert({ auth_uid: userData.user.id, username, role_id: 3, ref_id: student.id });
    if (appUserErr) return res.status(500).json({ error: appUserErr.message });
